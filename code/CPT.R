...
##         solve_optim_eta(X[ordering, ], m, ntestinds, M)$Ostar
##     }
##     ga_obj <- GA::ga("permutation", objective_fun, lower = rep(1, n), upper = rep(n, n), nBits = n, popSize = popSize, maxiter = rounds, run = rounds)
##     best_ordering <- as.integer(ga_obj@solution)
##     return(list(ordering = best_ordering,
##                 ga_obj = ga_obj))
## }
...
CPT <- function(y, X,
    ...
    y <- y[ordering]
    X <- X[ordering, ]
    eta <- solve_optim_eta(X, m, ntestinds)
    Piinds <- left_shift(n, m)
